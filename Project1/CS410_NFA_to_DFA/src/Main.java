...
//  tableTransitionsNFA();
    ...
    private static void newStateFinder() {
        ...
        for (String transition : transitionsNFA) {
            ...
            for (int j = 1; j < transitionsNFA.size(); j++) {
                System.out.println(transitionsNFA.get(j));//loop that checks the first element with every other element in the  transition array
